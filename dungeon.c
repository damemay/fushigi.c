...
fsg_dungeon* fsg_dungeon_init(fsg_config* config) {
    ...
    fsg_map_generate(d);
    d->minimap = fsg_minimap_create(d, d->config->minimap_tilesize);
    fsg_camera_update(d);
...
void fsg_map_clean(fsg_dungeon* dungeon) {
    for(uint8_t i=1; i<=dungeon->config->max_tiles; i++)
        if(dungeon->map->tiles[i]) {
            SDL_DestroyTexture(dungeon->map->tiles[i]);
        }
    if(dungeon->map->tiles) free(dungeon->map->tiles);
    if(dungeon->map->rooms) free(dungeon->map->rooms);

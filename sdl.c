...
static inline void sdl_init(const char* title,
    ...
    if(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER) < 0)
        fsg_logv("Could not init SDL: %s", SDL_GetError());

    if(!(*sdl_w = SDL_CreateWindow(title,
                    SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                    w, h, SDL_WINDOW_SHOWN)))
        fsg_logv("Could not create SDL_Window: %s", SDL_GetError());

    if(!(*sdl_r = SDL_CreateRenderer(*sdl_w, -1,
                    SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC)))
        fsg_logv("Could not create SDL_Renderer: %s", SDL_GetError());

    if(!(IMG_Init(IMG_INIT_JPG | IMG_INIT_PNG) & (IMG_INIT_JPG | IMG_INIT_PNG)))
        fsg_logv("Could not init SDL_image: %s", IMG_GetError());

    if(TTF_Init() == -1)
        fsg_logv("Could not init SDL_ttf: %s", TTF_GetError());
...
sdl_base* sdl_create(const char* title, const size_t w, const size_t h) {
    ...
    if(!base)
        fsg_log("Could not allocate memory for cdlv_base!");
